...
if (isset($_POST['admin-signup']))
{
	...
	if (!empty($admin_email) && !empty($password))
	{
		if (isset($_GET['id']))
		{
			...
			}elseif (IfExist(TBLADMIN, 'admin_email', $admin_email)) {
				$msg = "<div class='alert alert-info'><strong>".$admin_email."</strong> Already Exist,Please Choose Anothoer One.</div>";
			}else{
			echo $sql = "INSERT INTO `admin`(`full_name`, `admin_email`, `user_name`, `password`, `profile_picture`) 
		...
		if ($result)
		{
			...
			$msg = "<div class='alert alert-danger text-capitalize'>Errors occured! Admin name or admin email might me unavailable</div>";
		}
	}else{
						...
						<div class="row">
							<div class="col">
								...
								<form name="admin-signup" method="post" action="" onsubmit="return checkForm(this);" enctype="multipart/form-data">
									...
									<input title="Enter your admin name" type="text" value="<?php echo $user_name; ?>" name="user_name" class="blank" required />
