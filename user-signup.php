	...
	if (!empty($email_id) && !empty($password)){
		if (isset($_GET['id']))
		{
			...
			$sql = "INSERT INTO `tblusers`(`full_name`, `email_id`, `mobile_no`, `user_name`, `password`, `profile_picture`) 
				VALUES ('{$full_name}','{$email_id}','{$mobile_no}', '{$user_name}', '{$password}','{$profile_picture}')";
		}
		...
		{
			$msg = "<div class='alert alert-success text-capitalize'>Updation successful!</div>";	
		}else{
						...
						<div class="row">
							<div class="col">
								...
								<form class="" name="user-signup" method="post" action="#" enctype="multipart/form-data" onsubmit="return checkForm(this);">
									...
									<input title="Upload Profile Picture, not more than 11 MB" type="file" accept="img/*" value="<?php echo $profile_picture ?>" name="profile_picture" class="labelenglish text-uppercase" />
									...
									<input title="Enter Your E-mail" type="email" value="<?php echo $email_id ?>" name="email_id" class="blank" required />
									<p class="labelenglish"><b>User Name:</b></p>
									<input title="Enter Desired User Name" type="text" value="<?php echo $user_name ?>" name="user_name" class="blank" required />
									
									<p class="labelenglish"><b>Password:</b></p> 
									<input title="Enter Password" value="<?php echo $password ?>" type="password"  name="password" class="blank" required />
									
									<p class="labelenglish"><b>Mobile No.:</b></p>
									<input title="Enter your Contact No." type="number" value="<?php echo $mobile_no ?>" name="mobile_no" placeholder="Without dashes or spaces" class="blank" required />
									<?php 	
									...
									<div class="form-check form-check-inline" name="status">
										<input class="form-check-input" type="radio" name="status" value="Active" <?php if ($status === 'Active') { echo "Checked"; } elseif ($status === 'Inactive') { echo " "; } else { echo " ";}?> />
										<label class="form-check-label" for="status">
											Active
										</label>
									</div>
									<div class="form-check form-check-inline" name="status">
										<input class="form-check-input" type="radio" name="status" value="Inactive" <?php if ($status === 'Inactive') { echo "Checked"; } elseif ($status === 'Active') { echo " "; } else { echo " ";}?> />
										<label class="form-check-label" for="status"> 
											Inactive
									  	</label>
									</div>
